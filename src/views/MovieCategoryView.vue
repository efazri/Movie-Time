      ...
      <div class="w-11/12 p-3">
        <div class="flex justify-between mb-8 mt-12">
          <div>
            ...
            <h1 class="text-white text-4xl font-bold">Movies</h1>
          </div>
        </div>
...
export default {
      ...
      if (this.movieList.length > 12) {
        const newMoviesData = this.movieList.filter(
          (el, i) =>
            ...
            el.type.toLocaleLowerCase() === "movie"
        );
  ...
  watch: {
    ...
    popularity(newData) {
      ...
      const payload = { order, type, endSlice: 12, category: "movie" };
      this.$store.dispatch("onFilterByPopularity", payload);
    },
  },
  created() {
    const newDataList = movieList.filter(
      (el) => el.type.toLocaleLowerCase() === "movie"
    );
